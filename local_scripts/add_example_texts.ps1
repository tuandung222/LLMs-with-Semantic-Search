...
foreach ($text in $texts) {
    ...
    $result = Invoke-RestMethod -Method Post -Uri "http://localhost:8000/process-text" -Body $body -ContentType "application/json"
    Write-Output "[$counter/$total] Added: $($text.Substring(0, [Math]::Min(40, $text.Length)))..."
